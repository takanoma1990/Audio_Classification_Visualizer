...
const sketch = (p) => {
    ...
    p.draw = () => {
        ...
        // ★★★ ここからカメラ座標をリアルタイムで出力するコード ★★★
        // 負荷を減らすため、10フレームごとに1回だけ情報を出力
        if (p.frameCount % 10 === 0) {
            if (p._renderer && p._renderer.camera) {
                const cam = p._renderer.camera;
                // console.log({
                //     eyeX: Math.round(cam.eyeX), // 整数に丸めて見やすくする
                //     eyeY: Math.round(cam.eyeY),
                //     eyeZ: Math.round(cam.eyeZ),
                //     centerX: Math.round(cam.centerX),
                //     centerY: Math.round(cam.centerY),
                //     centerZ: Math.round(cam.centerZ)
                // });
            }
        }
        ...
        currentHue = p.lerp(currentHue, targetHue, 0.6);
    ...
    // ★ 絵文字を生成、更新する関数
    function updateAndSpawnEmojis() {
        for (const name in categoryData) {
            const data = categoryData[name];
            // 閾値を超えたら絵文字を生成
            if (data.accumulatedScore > EMOJI_SPAWN_THRESHOLD) {
                const emojiChar = labelToEmojiMap[name];
                if (emojiChar) {
                    emojis.push(new EmojiObject(emojiChar));
                }
                // スコアを消費
                data.accumulatedScore -= EMOJI_SPAWN_THRESHOLD;
            }
            // スコアを時間で減衰させる
            data.accumulatedScore *= EMOJI_SCORE_DECAY;
        }
    }
    ...
    function drawTextOverlay() {
        ...
        const columnWidth = p.width / numColumns;
        const barHeight = 20;
        const lineHeight = 28;
        const titleSize = 40;
        const labelSize = 30;
    ...
    function drawSpectrogram(spectrum) {
        ...
        for (let i = 0; i < spectrumHistory.length - 1; i++) {
            ...
            for (let j = startBin; j < spectrumHistory[i].length; j += 8) {
                ...
                let alpha = p.map(v1.amplitude, noiseThreshold, 200, 0.01, 5.0, true);
        ...
        const x = p.map(i, 0, spectrumHistory.length - 1, p.width / 2, -p.width / 2);
    ...
    function drawWaveformLine() {
        ...
        if (isMicOn && fft) {
            ...
            const weight = p.map(smoothedBassLevel, 0, 1.5, 1, 3, true);
            ...
            for (let i = 0; i < waveform.length; i++) {
                ...
                let y = p.map(smoothedWaveform[i], -0.5, 0.5, -6000, 6000);
    ...
    class EmojiObject {
        constructor(emojiChar) {
            ...
            this.size = p.random(80, 120);
            this.pos = p.createVector(
                p.random(-p.width / 2, p.width / 2),
                p.random(-p.height / 2, p.height / 2),
                p.random(-1000, 1000)
            );
            this.vel = p.createVector(p.random(-5, 5), p.random(-1.5, 1.5), p.random(-1.5, 1.5));
            this.lifespan = 255.0; 

            this.texture = p.createGraphics(this.size, this.size);
            this.texture.pixelDensity(1);
            this.texture.textFont("Noto Color Emoji");
            this.texture.textSize(this.size * 0.8);
            this.texture.textAlign(p.CENTER, p.CENTER);
        ...
        display() {
            this.texture.clear();
            this.texture.fill(255, this.lifespan); 
            this.texture.text(this.emoji, this.size / 2, this.size / 2);
    ...
    class Particle {
        ...
        display() {
            ...
            let s = this.size + smoothedBassLevel * 15; // 描画サイズを変数sにまとめる
            ...
            switch (this.shapeType) {
                ...
                case 2: // カスタムの三角錐
                    ...
                    // 4つの頂点を定義
                    let v0 = p.createVector(0, -s, 0);       // てっぺん
