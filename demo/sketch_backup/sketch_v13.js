...
const sketch = (p) => {
    ...
    p.preload = () => {
        ...
        soundFile = p.loadSound('music/test_v12.mp3');
    ...
    p.setup = async () => {
        ...
        textLayer = p.createGraphics(p.windowWidth, 400);
        ...
        // ★ MediaPipeのセットアップと並行して、ノイズリデューサーも初期化
        noiseReducer = new NoiseReducer();
        await noiseReducer.initialize();
         isAIReady = noiseReducer.isInitialized;
        ...
        if (audioClassifier) {
            ...
            // FFT（視覚エフェクト用）は元のサウンドファイルを入力とする
            fft.setInput(soundFile);

            // ★★★★★ 不要になったnoiseGate関連のコードを完全に削除します ★★★★★
            // soundFile.connect(noiseGate);  <-- 削除
            // fft.setInput(noiseGate);       <-- 削除

            scriptNode = audioCtx.createScriptProcessor(16384, 1, 1);
            
            // `onaudioprocess` の中身は前回の修正でOKです
            scriptNode.onaudioprocess = (e) => {
                if (!isPlaying || !audioClassifier || !isAIReady || audioCtx.state !== 'running'){
                    console.log("処理スキップ:", { isPlaying, isAIReady }); 
                    return;
                }
                ...
                // 2.【ステップA】AIでノイズ除去を実行し、`cleanData`を作成
                const cleanData = noiseReducer.process(inputData);

                console.log("AI処理後のデータ (cleanData):", cleanData);

                // 3.【ステップB】"クリーンなデータ"をMediaPipeで分類
                const results = audioClassifier.classify(cleanData, audioCtx.sampleRate);

                console.log("MediaPipeの分類結果 (results):", results);
                ...
                // 5.【ステップC】処理後の音をスピーカーから出すために出力バッファにコピー
            ...
            // ★★★★★ オーディオ接続を正しく設定します ★★★★★
            // soundFileの音を、AI処理を行うscriptNodeに接続する
            soundFile.connect(scriptNode);
            // scriptNodeから出てきた音（ノイズ除去後）を、最終的な出力（スピーカー）に接続する
