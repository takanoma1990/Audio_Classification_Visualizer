...
const sketch = (p) => {
  ...
  let majorCategoryScores = {
   ...
   // レーダーチャート用：0〜1の表示比率をスムージングして持つ
  let radarDisplayRatios = {
    "Forest & Life": 0,
    Water: 0,
    Atmosphere: 0,
    Traffic: 0,
  ...
  function updateHueAndMode() {
    ...
    const circleSmooth = 0.001; // 0〜1（小さいほどゆっくり）
    ...
    for (const majorCategory in CATEGORIES_HIERARCHY) {
      ...
      // ★ レーダーチャート用：サークルに使う値をさらにスムージング
      const prev = majorCategoryScores[majorCategory] || 0;
      majorCategoryScores[majorCategory] = p.lerp(prev, majorCategoryScore, circleSmooth);
